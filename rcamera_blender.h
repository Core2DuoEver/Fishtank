...
{
    bcamera->camera = {0};
    bcamera->camera.position = {10.0f, 10.0f, 10.0f};
    bcamera->camera.target = {0.0f, 0.0f, 0.0f};
    bcamera->camera.up = {0.0f, 1.0f, 0.0f};
    bcamera->camera.fovy = 45.0f;
    bcamera->camera.projection = CAMERA_PERSPECTIVE;
    bcamera->previousMousePosition ={0};
...
void BlenderCameraUpdate(BlenderCamera *bcamera)
{
    ...
    if (!bcamera->freeFly)
    {
        ...
        if (middleMouseDown)
        {
            ...
            {
                // Rotate the camera
                Quaternion qYaw = QuaternionFromAxisAngle({0.0f, 1.0f, 0.0f}, mouseDelta.x * bcamera->rotationSpeed);
